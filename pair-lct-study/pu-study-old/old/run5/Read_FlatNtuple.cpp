...
TH1D *SingleMuPhiPairsInTracks =
    new TH1D("SingleMuPhiPairsInTracks", "SingleMuPairsInTracks", 10, 0, 10);
...
TH2D *SingleMuPhiPairInTrack =
    new TH2D("SingleMuTrackPhiPair", "SingleMuTrackPhiPair", 54, 1, 55, 12, -6, 6);
TH2D *SingleMuThetaPairOccupancy = new TH2D(
    "ThetaPairChamberOccupancy", "ThetaPairChamberOccupancy", 54, 1, 55, 12, -6, 6);
TH2D *SingleMuThetaPairInTrack =
    new TH2D("SingleMuTrackThetaPair", "SingleMuTrackThetaPair", 54, 1, 55, 12, -6, 6);
  ...
  for (int iEvt = 0; iEvt < nEvents; iEvt++) {
    ...
    for (int trk = 0; trk < I("nUnpTracks"); trk++) {
      ...
      if (/*I("unp_trk_nRPC",i)==0 &&*/ I("unp_trk_nHits", trk) >= 3) {
        ...
        if (!(trackUsesPhiPair || trackUsesThetaPair)) {
    ...
    // Fill the Phi Pair in track occupancy plot
